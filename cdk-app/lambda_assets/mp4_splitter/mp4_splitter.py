...
def get_video_duration(url):
    #ffprobe = subprocess.run(['ffprobe', '-loglevel', 'error', '-show_streams', str(url), '-print_format', 'json'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    duration = subprocess.run(
        ['ffprobe', '-i', str(url), '-show_entries', 'format=duration', '-v', 'quiet', '-of', 'csv=%s' % ("p=0")], 
